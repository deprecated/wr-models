...
for Tstar in Tstar_range:
    for loghden in hdenrange:
        for logphi in phirange:
            outfile = file("coolfunc-%s-tstar%3.3i-phi%.2f-%s-n%.2f.dat" % (
                    ionized, Tstar/1000, logphi, abunid, loghden), "w")
            outfile.write('# Temperature\tLambda (erg cm3/s)\tNp\tNe\tL (erg/cm3/s)\tH (erg/cm3/s)\tTop Cooler\tfrac\tTop Heater\tfrac\n')
            for logT in Trange:
                filename = "%s-tstar%3.3i-phi%.2f-%s-n%.2f-T%.2f" % (
                    ionized, Tstar/1000, logphi, abunid, loghden, logT)
                ...
                try: 
                    ...
                    fields = f.readlines()[-1].split('\t')
                    pden = float(fields[3])*float(fields[7])
                    hden = float(fields[3])
                    eden = float(fields[4])
                    cool = Ctot/(hden*eden)
                except:
